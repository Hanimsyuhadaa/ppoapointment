 ...
 * @var iterable<\App\Model\Entity\Appoinment> $appoinments
 */
		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Appoinment'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
				...
				<td><?php echo $counter++ ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $appoinment->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $appoinment->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							<?php $this->Form->setTemplates([
								'confirmJs' => 'addToModal("{{formName}}"); return false;'
							]); ?>
							<?= $this->Form->postLink(
								__('<i class="fa-regular fa-trash-can"></i>'),
								['action' => 'delete', $appoinment->id],
								[
									'confirm' => __('Are you sure you want to delete Appoinments: "{0}"?', $appoinment->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				<div class="col-md-4">
				  <div class="stat_card card-1 bg-body-tertiary">
					<h3><?php echo $total_appoinments; ?></h3>
					<p>Total Appoinments</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_appoinments_active; ?></h3>
					<p>Active Appoinments</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_appoinments_archived; ?></h3>
					<p>Archived Appoinments</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Appoinments (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Appoinments by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Appoinments(s)',
			data: [<?= json_encode($total_appoinments_active); ?>, <?= json_encode($total_appoinments_disabled); ?>, <?= json_encode($total_appoinments_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Appoinments','action' => 'index']]); ?>
				<fieldset>
